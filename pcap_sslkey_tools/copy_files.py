...
def build_tasks_for_date(roots: Sequence[str], ymd: str, dest_root: str) -> List[Tuple[str, str]]:
    """
    仅匹配形如：
      <root>/content/YYYYMMDD/*/*.txt
      <root>/data/YYYYMMDD/*/*.pcap
      <root>/ssl_key/YYYYMMDD/*/*.log
      <root>/html/YYYYMMDD/*/*.html
      <root>/screenshot/YYYYMMDD/*/*.png
    返回 (src_file, dest_dir) 列表
    """
    tasks: List[Tuple[str, str]] = []
    for root in roots:
        # content
        for f in list_files(f"{root}/content/{ymd}/*/*.txt"):
            tasks.append((f, f"{dest_root}/content"))
        # pcap（源目录名为 data）
        for f in list_files(f"{root}/data/{ymd}/*/*.pcap"):
            tasks.append((f, f"{dest_root}/pcap"))
        # ssl_key
        for f in list_files(f"{root}/ssl_key/{ymd}/*/*.log"):
            tasks.append((f, f"{dest_root}/ssl_key"))
        # html
        for f in list_files(f"{root}/html/{ymd}/*/*.html"):
            tasks.append((f, f"{dest_root}/html"))
        # html
        for f in list_files(f"{root}/screenshot/{ymd}/*/*.png"):
            tasks.append((f, f"{dest_root}/screenshot"))
    return tasks
