                    ...
                    if hasattr(packet,
                        ...
                        if sni:
                            ...
                            if 'zlfbgac.site' in sni:
                                ...
                                capture.close()
                                os.remove(file_path)
                                print(f'Removed noVPN Pcap: {file_path}')
                                # Check if the directory still contains any pcap files
                                file_directory = os.path.dirname(file_path)
                                if not any(file.endswith('.pcap') for file in os.listdir(file_directory)):
                                    os.rmdir(file_directory)
                                    print(f'Removed empty directory: {file_directory}')
                                return None
...
if __name__ == "__main__":
    # 解析命令行参数
    directory = r'/netdisk/dataset/vpn/data'
